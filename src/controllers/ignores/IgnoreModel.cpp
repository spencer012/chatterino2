...
IgnoreModel::IgnoreModel(QObject *parent)
    : SignalVectorModel<IgnorePhrase>(8, parent)
{
}
                                         ...
                                         const IgnorePhrase &original)
{
    ...
    return IgnorePhrase{row[0]->data(Qt::DisplayRole).toString(),
                        ...
                        row[7]->data(Qt::DisplayRole).toString(),
                        row[2]->data(Qt::CheckStateRole).toBool(),
                        row[4]->data(Qt::CheckStateRole).toBool(),
                        row[5]->data(Qt::CheckStateRole).toBool(),
                        row[6]->data(Qt::CheckStateRole).toBool()};
}
                                 ...
                                 std::vector<QStandardItem *> &row)
{
    ...
    setBoolItem(row[4], item.highlightOnly());
    setBoolItem(row[5], item.suppressMentions());
    setBoolItem(row[6], item.suppressHighlight());
    setStringItem(row[7], item.getReplace());
}
